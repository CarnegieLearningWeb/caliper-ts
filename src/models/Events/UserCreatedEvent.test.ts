...
describe('UserCreatedEvent', () => {
	...
	const userEvent = {
		...
		type: 'UserEvent',
		action: 'Created',
		eventTime: '2021-01-01T12:30:00.000Z',
		edApp: {
		...
		object: {
			...
			dateModified: '2021-01-01T12:30:00.000Z',
			status: 'Active',
			otherIdentifiers: [
				{
	...
	it('OK', () => {
		...
		expect(model).toEqual(userEvent);
	});
	...
	it('OK Student', () => {
		...
		model.id = Caliper.uuid('e251d4a0-b93c-4a0e-86cc-8b14c8db6787');
		model.eventTime = model.object.dateModified;
		expect(model).toEqual(studentEvent);
	});
	...
	const instructorEvent = {
		...
		object: {
			...
			permissions: {
				whateverPermission: true,
				whateverSomething: 'value',
			},
		},
	};
	...
	it('OK Instructor', () => {
		const model = createUserCreatedEvent({
			actor: createUser({ id: Caliper.uuid('f41448d9-76e5-4171-8e0a-3e0e237a8581') }),
			object: createUserEventInstructor({
				id: Caliper.uuid('a1d5c07b-688c-41aa-b492-30c67084c8cc'),
				dateCreated: Caliper.timestamp('2021-01-01T12:30:00Z'),
				dateModified: Caliper.timestamp('2021-01-01T12:30:00Z'),
				status: Status.Active,
		...
		model.id = Caliper.uuid('e251d4a0-b93c-4a0e-86cc-8b14c8db6787');
		model.eventTime = model.object.dateModified;
		expect(model).toEqual(instructorEvent);
	});
});
